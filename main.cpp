...
string filter_list[MAX_LENGTH];

void read_filter_list(char* filepath){
	...
	ifstream openFile(filepath);
		if(openFile.is_open()){
			...
			while(getline(openFile, line)){
				filter_list[i++]=StringSplit(line,",")[1];
				if(i==MAX_LENGTH){
					return ;
				}
			}
...
int main(int argc, char **argv)
{
	...
	if(argc!=2){
		printf("usage: 1m_block 1m-top.txt\n");
		return -1;
	}else{
		filepath  = argv[1];
		read_filter_list(filepath);
	}
